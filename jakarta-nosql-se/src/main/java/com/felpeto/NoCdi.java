...
public class NoCdi {
  ...
  public static void main(String[] args) {
    ...
    List<DocumentConfiguration> providers = new java.util.ArrayList<>(
        ServiceLoader.load(DocumentConfiguration.class).stream()
            .map(ServiceLoader.Provider::get).toList());

    settings.get(MappingConfigurations.DOCUMENT_PROVIDER, String.class)
        .ifPresent(providerClassName -> providers.removeIf(provider ->
            !providerClassName.equals(provider.getClass().getName())));

    var database = settings.get(MappingConfigurations.DOCUMENT_DATABASE, String.class).orElse(null);
    Objects.requireNonNull(database,
        MappingConfigurations.DOCUMENT_DATABASE.get() + " property is missing");
